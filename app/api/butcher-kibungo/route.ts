...
export async function POST(request: Request) {
  try {
    ...
    if (existingRowIndex > -1) {
      await updateRow('butcher-kibungo', existingRowIndex, rowData);
    } else {
...
export async function GET(request: Request) {
  try {
    ...
    if (lastStock === '1') {
      ...
      if (forDate) {
        const targetDateStr = toDateStr(forDate); // normalized YYYY-MM-DD

        // Normalize all row dates using local time so '5/14/2026' matches '2026-05-14'
